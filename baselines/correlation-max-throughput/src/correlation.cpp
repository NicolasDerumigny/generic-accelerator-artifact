...
#		pragma HLS pipeline ii=1
	    ...
	    for (i = 0; i < N ; i++)
	        for (j = 0; j < N ; j++) { // accsumcm mean, data
#		pragma HLS dependence dependent=false type=inter variable=reg_file
#		pragma HLS unroll factor=2
#		pragma HLS pipeline ii=1
	        	half val1 = mean[j];
	        	half val2 = data[access(i,j)];
	        	mean[j] = val1 + val2;
	        }
...
#		pragma HLS unroll factor=2
#		pragma HLS pipeline ii=1
	        mean[j] /= float_n; // divms mean, n

	    for (j = 0; j < N ; j++) // mv stddev, 0
#		pragma HLS dependence dependent=false type=inter variable=reg_file
#		pragma HLS unroll factor=2
#		pragma HLS pipeline ii=1
	        stddev[j] = 0.0;
	    ...
	    for (i = 0; i < N ; i++)
	        for (j = 0; j < N; j++) { //subcmv data, data, mean
#		pragma HLS dependence dependent=false type=inter variable=reg_file
#		pragma HLS unroll factor=2
#		pragma HLS pipeline ii=1
	        	half val1 = data[access(i,j)];
	        	half val2 = minus(mean[j]);
	        	data[access(i,j)] = val1 + val2;
	        }

	    for (j = 0; j < N ; j++)
	    	for (i = 0; i < N; i++) {// pmulm, tmp1, data, data
#		pragma HLS dependence dependent=false type=inter variable=reg_file
#		pragma HLS unroll factor=2
#		pragma HLS pipeline ii=1
	        	half val1 = data[access(i,j)];
	        	tmp1[access(i,j)] = val1 * val1;
	    	}

		for (i = 0; i < N ; i++)
			for (j = 0; j < N; j++) { // accsumcm, stddev, tmp1
#		pragma HLS dependence dependent=false type=inter variable=reg_file
#		pragma HLS unroll factor=2
#		pragma HLS pipeline ii=1
	        	half val = tmp1[access(i,j)];
	        	stddev[j] += val;
			}

	    for (j = 0; j < N ; j++) { // divvs stddev, n
#		pragma HLS dependence dependent=false type=inter variable=reg_file
#		pragma HLS unroll factor=2
#		pragma HLS pipeline ii=1
        	half val1 =  stddev[j];
	    ...
	    for (j = 0; j < N ; j++) { // psqrtv stddev, stddev
#		pragma HLS dependence dependent=false type=inter variable=reg_file
#		pragma HLS unroll factor=2
#		pragma HLS pipeline ii=1
		...
		for (j = 0; j < N ; j++) // roundv1 stddev, stddev, eps
#		pragma HLS dependence dependent=false type=inter variable=reg_file
#		pragma HLS unroll factor=2
#		pragma HLS pipeline ii=1
			stddev[j] = stddev[j] <= CUTOFF ? 1.0 : stddev[j];

		for (j = 0; j < N ; j++) // mulsv tmp1, sqrt n, stddev
#		pragma HLS dependence dependent=false type=inter variable=reg_file
#		pragma HLS unroll factor=4
#		pragma HLS pipeline ii=1
				tmp1[j] = sqrtfloat_n * stddev[j];

		for (i = 0; i < N ; i++)
			for (j = 0; j < N ; j++) { // divcmv data, tmp1
#		pragma HLS dependence dependent=false type=inter variable=reg_file
#		pragma HLS unroll factor=2
#		pragma HLS pipeline ii=1
				half val1 = data[access(i,j)];
...
#		pragma HLS pipeline ii=1
		...
		for (i = 0; i < N; i++) // trm tmp1, data
			for (j =0; j < N ; j++)
#		pragma HLS dependence dependent=false type=inter variable=reg_file
#		pragma HLS unroll factor=2
#		pragma HLS pipeline ii=1
				tmp1[access(i,j)] = data[access(j,i)];

		for (i = 0; i < N; i++)
			for (j = 0; j < N; j++)
				for (k = 0; k < N; k++) { // mulmm corr, tmp1, data
#		pragma HLS dependence dependent=false type=inter variable=reg_file
#		pragma HLS unroll factor=2
#		pragma HLS pipeline ii=1
					half val = corr[access(i,k)];
					half ret;
					if (j == 0)
						ret = 0;
					else
						ret = val;
					ret += (tmp1[access(i,j)] * data[access(j,k)]);
					corr[access(i,k)] = ret;
				}

		for (i = 0; i < N  - 1; i++) // diag corr, 1
#		pragma HLS dependence dependent=false type=inter variable=reg_file
#		pragma HLS unroll factor=4
#		pragma HLS pipeline ii=1
			corr[access(i,i)] = 1.0;
